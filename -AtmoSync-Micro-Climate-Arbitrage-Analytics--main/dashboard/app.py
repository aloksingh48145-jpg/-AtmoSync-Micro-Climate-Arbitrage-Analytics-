...
from risk_logic import compute_risk_level, compute_temperature_status, arbitrage_recommendation
...
if len(shipments_df):
    shipments_df["risk_level"] = shipments_df.apply(
        lambda row: compute_risk_level(row["peak_severity_score"], row["has_high_risk_window"]), axis=1
    )
...
# ---------- Arbitrage View page ----------
    ...
    st.divider()
    st.subheader("🔍 Why is this shipment flagged?")
    if len(filtered):
        explain_shipment = st.selectbox(
            "Select a shipment to see the recommendation reasoning",
            filtered["shipment_id"].unique().tolist(),
            key="explain_shipment",
        )
        row = filtered[filtered["shipment_id"] == explain_shipment].iloc[0]
        rec = arbitrage_recommendation(row["peak_severity_score"], row["pct_readings_out_of_band"])

        badge_color = {"High": "🔴", "Medium": "🟠", "Low": "🟢"}[rec["risk_level"]]
        st.markdown(f"**{badge_color} {explain_shipment} — Recommended action: {rec['action']}**")
        st.write(rec["reason"])
